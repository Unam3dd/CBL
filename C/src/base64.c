...
{
    uint32_t bytes[3] = {0};
    uint64_t size_of_buffer = strlen(buffer);
    uint32_t sum = 0;
...
uint32_t base64_decode(char *buffer, char *output, uint64_t len_output_buffer)
{
    ...
    if (get_ascii_size_from_base64(strlen(buffer)) > len_output_buffer)
        return (1); // return error
    
    uint32_t bytes[4] = {0}; // bytes in 
    uint64_t sum = {0};
    uint64_t size_of_buffer = strlen(buffer);
    uint64_t size_of_output_buffer = get_ascii_size_from_base64(strlen(buffer));
...
void make_decoding_table(void)
{
    ...
    for (int i = 0; i < 0x40; i++)
        DECODING_TABLE[(uint8_t) BASE64_TABLE[i]] = i; // example : A = 65, DECODING_TABLE[65] = 1;
}
